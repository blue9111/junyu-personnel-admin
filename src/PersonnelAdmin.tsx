...
export default function PersonnelAdmin({user}:{user:User}) {
  ...
  return <section className="personnel-panel">
    ...
    {directoryForm&&<div className="personnel-modal-backdrop" role="presentation" onKeyDown={e=>{if(e.key==='Escape'&&!busy)setDirectoryForm(null);}}>
      <form className="personnel-modal" role="dialog" aria-modal="true" aria-labelledby="directory-edit-title" onSubmit={e=>{e.preventDefault();void saveDirectory();}}>
        <div className="personnel-modal-head"><div><small>EMPLOYEE PROFILE</small><h2 id="directory-edit-title">編輯人員資料</h2></div><button type="button" aria-label="關閉編輯視窗" disabled={busy} onClick={()=>setDirectoryForm(null)}>×</button></div>
        {directoryForm.email&&<p className="personnel-linked-account">已連結帳號：<strong>{directoryForm.email}</strong></p>}
        <fieldset disabled={busy}><div className="form-grid directory-form-grid"><label>工號<input autoFocus required maxLength={40} value={directoryForm.employeeId} onChange={e=>setDirectoryForm({...directoryForm,employeeId:e.target.value})}/></label><label>姓名<input required maxLength={80} value={directoryForm.name} onChange={e=>setDirectoryForm({...directoryForm,name:e.target.value})}/></label><label>部門代號<input required maxLength={20} value={directoryForm.departmentCode} onChange={e=>setDirectoryForm({...directoryForm,departmentCode:e.target.value})}/></label><label>單位<input required maxLength={80} value={directoryForm.unit} onChange={e=>setDirectoryForm({...directoryForm,unit:e.target.value})}/></label><label className="personnel-job-title">職稱<input required maxLength={80} value={directoryForm.jobTitle} onChange={e=>setDirectoryForm({...directoryForm,jobTitle:e.target.value})}/></label></div><p className="personnel-modal-note">變更工號時會保留舊工號的稽核紀錄，並將已連結帳號移至新工號。</p><div className="personnel-modal-actions"><button type="button" onClick={()=>setDirectoryForm(null)}>取消</button><button className="primary" type="submit">{busy?'儲存中…':'儲存資料'}</button></div></fieldset>
      </form>
    </div>}
